...
/**
 * HTML shell for the standalone webview.
 *
 * This is the same HTML structure as panel-html.ts, but with two key differences:
 * 1. Uses relative paths for assets (no vscode.Uri needed)
 * 2. Includes CSS variable fallbacks so the UI works outside VS Code
 *
 * The webview app.js will detect it's not in VS Code and use fetch() for RPC.
 */

/**
 * Generate the HTML shell for the standalone dashboard.
 *
 * @param featureFlags - Optional feature flags to control UI elements
 * @returns Complete HTML string
 */
export function getStandaloneHtml(featureFlags?: { tokenReportingEnabled?: boolean }): string {
  const tokenEnabled = featureFlags?.tokenReportingEnabled ?? false;
...
<style>
/* CSS variable fallbacks for standalone mode */
/* These mirror VS Code's theme variables but with sensible defaults */
:root {
  ...
  --vscode-progressBar-background: #58a6ff;
  --accent-blue: #58a6ff;
  --border: #474747;
}
body {
    ...
    <div class="sidebar-filters">
      <div class="sidebar-filter">
        ...
        <div class="ws-toggle" id="ws-toggle">
          <button class="ws-toggle-btn active" data-ws="all">All</button>
        </div>
...
/**
 * Generate an error page for the standalone server.
 */
